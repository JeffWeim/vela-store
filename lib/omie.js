...
export const putCustomer = async ({
  ...
  const createCustomerBody = {
    ...
    call: 'UpsertCliente',
    param: [{
      'codigo_cliente_omie': parseInt(extId),
      'codigo_cliente_integracao': extId,
      'email': email,
      'razao_social': name,
      'cnpj_cpf': doc,
      'telefone1_ddd': phone.split('').filter(Number).join('').substring(0, 2),
      'telefone1_numero': phone.split('').filter(Number).join('').substring(3, phone.length),
      'cep': zip,
      'estado': state,
      'cidade': city,
      'endereco': address,
  ...
  const createOrderBody = {
    ...
    param: [{
      cabecalho: {
        ...
        codigo_cliente: parseInt(customerId),
        codigo_cliente_integracao: customerId,
        codigo_pedido: parseInt(orderId),
        codigo_pedido_integracao: orderId
      },
