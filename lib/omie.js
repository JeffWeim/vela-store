...
export const addProduct = async ({ page }) => {
  const endpointURL = apiURL + '/geral/produtos/'
  const body = {
    ...auth,
    call: 'IncluirProduto',
    param: [{
      'codigo_pedido_integracao': page.id,
  ...
  const response = await fetch(endpointURL, options)
  //const { faultstring, ...addProductResponse } = await response.json()
  // if (faultstring) throw new Error(faultstring)

  // return normalizeProduct(addProductsResponse)
}
...
export const listProducts = async ({ page }) => {
  const endpointURL = apiURL + '/geral/produtos/'
  const body = {
    ...auth,
    call: 'ListarProdutos',
  ...
  const response = await fetch(endpointURL, options)
  const { faultstring, ...listProductsResponse } = await response.json()
  if (faultstring) throw new Error(faultstring)

  return normalizeListCustomersResponse(listProductsResponse)
}
